...
public class TodoResource {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<TodoDTO> getTodo(@PathVariable(name = "id") final Integer id) {
        return ResponseEntity.ok(todoService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<Integer> createTodo(@RequestBody @Valid final TodoDTO todoDTO) {
        final Integer createdId = todoService.create(todoDTO);
        return new ResponseEntity<>(createdId, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Integer> updateTodo(@PathVariable(name = "id") final Integer id,
    ...
    public ResponseEntity<Void> deleteTodo(@PathVariable(name = "id") final Integer id) {
